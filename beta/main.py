...
def loft_shapes(airfoils, z_values, layer_height, infill_density, generate_infill, generate_circle, circle_centers, circle_radiuses, circle_num_points, infill_type, infill_reverse, circle_offset, circle_segment_ratio, circle_start_angle):
    ...
    for layer in layers:
        ...
        if generate_circle:
            circle_layers = create_circles(circle_centers, circle_radiuses, circle_offset, layer[0].z, circle_num_points)
...
num_points = 256 # The resolution / accuracy of your airfoil (and circle by default).
...
# Wing parameters
z_values = [0, layer_height]  # List of z-values for the airfoils
...
circle_centers = [
    [fc.Point(x=75, y=0, z=min(z_values)), fc.Point(x=75, y=0, z=max(z_values))]
]
circle_radiuses = 5
